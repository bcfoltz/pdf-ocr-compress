...
def unique_output_path(base_path: Union[str, Path], suffix: str = "_processed") -> Path:
    ...
    candidate = base_path.with_name(f"{base_path.stem}{suffix}_{timestamp}{base_path.suffix}")
    
    counter = 0
    while candidate.exists():
        counter += 1
        candidate = base_path.with_name(f"{base_path.stem}{suffix}_{timestamp}_{counter}{base_path.suffix}")
